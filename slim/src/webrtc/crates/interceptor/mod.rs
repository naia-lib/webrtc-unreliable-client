...
pub type RTPWriterBoxFn = Box<
    dyn (Fn(
            &crate::webrtc::crates::rtp::packet::Packet,
            &Attributes,
...
impl RTPWriter for RTPWriterFn {
    /// write a rtp packet
    async fn write(&self, pkt: &crate::webrtc::crates::rtp::packet::Packet, attributes: &Attributes) -> Result<usize> {
...
pub trait RTCPWriter {
    ...
    async fn write(
        &self,
        pkts: &[Box<dyn crate::webrtc::crates::rtcp::packet::Packet + Send + Sync>],
...
pub type RTCPWriterBoxFn = Box<
    dyn (Fn(
            &[Box<dyn crate::webrtc::crates::rtcp::packet::Packet + Send + Sync>],
            &Attributes,
        ) -> Pin<Box<dyn Future<Output = Result<usize>> + Send + Sync>>)
        + Send
        + Sync,
